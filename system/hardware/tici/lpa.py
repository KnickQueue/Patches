...
class AtClient:
  ...
  def _send_csim_apdu(self, apdu: bytes) -> tuple[bytes, int, int]:
    hex_payload = apdu.hex().upper()
    for line in self.query(f'AT+CSIM={len(hex_payload)},"{hex_payload}"'):
...
class TiciLPA(LPABase):
  ...
  def _open_client(self):
    client = None
    restart_modemmanager = False
    try:
      # Force exclusive mode if env requests it or if AGNOS udev workaround
      # (PR 521) is absent on EG25 ttyUSB2.
      force_exclusive = os.getenv(ESIM_EXCLUSIVE_MODEM_ENV, "").strip() == "1" or not self._ttyusb2_mm_ignored()
      if self.verbose and force_exclusive:
        print("!! enabling exclusive modem mode for eSIM session", file=sys.stderr)
      if force_exclusive and self._is_modemmanager_running():
        self._set_modemmanager_running(False)
        restart_modemmanager = True
        time.sleep(1.0)
      client = self._open_ready_client()
      yield client
    finally:
      if client is not None:
        client.close()
      if restart_modemmanager:
        self._set_modemmanager_running(True)
        time.sleep(1.0)
  ...
  def _candidate_devices(self) -> list[str]:
    ...
    for dev in candidates:
      if dev and dev not in ordered and os.path.exists(dev):
        ordered.append(dev)
  ...
  @staticmethod
  def _restart_modem() -> None:
    from openpilot.system.hardware import HARDWARE
    HARDWARE.reboot_modem()
    # eUICC takes a short time post-reboot to reflect state.
    time.sleep(0.5)
  ...
  def list_profiles(self) -> list[Profile]:
    def _run() -> list[Profile]:
      with self._open_client() as client:
        return [self._to_profile(p) for p in list_profiles(client)]
  ...
  def switch_profile(self, iccid: str) -> None:
    ...
    def _run() -> None:
      with self._open_client() as client:
        enable_profile(client, iccid, refresh=True)
        process_notifications(client)
      self._restart_modem()
    self._with_lpa_error(_run)
