...
class VCruiseHelperIQ:
  ...
  def update_speed_limit_assist_v_cruise_op_long(self) -> None:
    ...
    # Keep SLC's resolved limit as a persistent upper bound on OP Long.
    # This allows initial engagement/limit changes to move the set speed,
    # while also re-clamping manual scroll-ups that exceed limit+offset.
    should_sync_to_target = (
      self.speed_limit_state in SPEED_LIMIT_CONTROL_ACTIVE_STATES or
      self.update_speed_limit_final_last_changed or
      (self.v_cruise_kph != V_CRUISE_UNSET and self.v_cruise_kph > target_kph) or
      (self.v_cruise_cluster_kph != V_CRUISE_UNSET and self.v_cruise_cluster_kph > target_kph)
    )

    if should_sync_to_target:
