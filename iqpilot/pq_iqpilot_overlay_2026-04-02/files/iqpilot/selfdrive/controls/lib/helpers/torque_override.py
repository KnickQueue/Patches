...
PQ_CORNER_BOOST_MAX = 0.17
PQ_CORNER_BOOST_SPEED_MPS = 12.0
PQ_CORNER_BOOST_CURVATURE = 0.012
...
class LatControlTorqueExtOverride:
  ...
  def update_low_speed_corner_boost(self,
                                    active: bool,
                                    car_state,
                                    desired_curvature: float,
                                    torque_output: float) -> float:
    if not active or getattr(car_state, "steeringPressed", False):
      return torque_output

    if not bool(getattr(self._profile, "flags", 0) & VolkswagenFlags.PQ):
      return torque_output

    v_ego = float(getattr(car_state, "vEgo", 0.0))
    if v_ego >= PQ_CORNER_BOOST_SPEED_MPS:
      return torque_output

    curvature = abs(float(desired_curvature))
    if curvature <= 0.0:
      return torque_output

    speed_scale = np.interp(v_ego, [0.0, PQ_CORNER_BOOST_SPEED_MPS], [1.0, 0.0])
    curvature_scale = np.interp(curvature, [0.0, PQ_CORNER_BOOST_CURVATURE], [0.0, 1.0])
    boost = 1.0 + (PQ_CORNER_BOOST_MAX * speed_scale * curvature_scale)
    steer_max = float(getattr(self._controller, "steer_max", 1.0))
    return float(np.clip(torque_output * boost, -steer_max, steer_max))
