...
class CarController(CarControllerBase):
  def __init__(self, dbc_names, CP, CP_IQ):
    ...
    self.accel_last = 0
    self.accel_diff = 0
    self.long_deviation = 0.02
    self.long_jerklimit = 0.7
    self.HCA_Status = 3
  ...
  def update(self, CC, CC_IQ, CS, now_nanos):
    ...
    pq_long_profile = int(self._params.get("VolkswagenPqLongitudinalProfile", return_default=True) or 1)
    ...
    if self.frame % self.CCP.ACC_CONTROL_STEP == 0 and self.CP.openpilotLongitudinalControl:
      ...
      if self.CP.flags & (VolkswagenFlags.MEB | VolkswagenFlags.MQB_EVO):
        ...
        # PQ comfort profiles tune accel filtering and jerk windows without touching steering safety limits.
        if pq_long_profile <= 0:  # Comfort
          lpf_accel = np.interp(CS.out.vEgo, [0, 10, 20], [0.005, 0.003, 0.0015])
          lpf_jerk = np.interp(CS.out.vEgo, [0, 10, 20], [0.1, 0.05, 0.01])
          deviation_curve = [0.02, 0.035, 0.075]
        elif pq_long_profile == 1:  # Balanced
          lpf_accel = np.interp(CS.out.vEgo, [0, 10, 20], [0.008, 0.0045, 0.0022])
          lpf_jerk = np.interp(CS.out.vEgo, [0, 10, 20], [0.14, 0.08, 0.015])
          deviation_curve = [0.02, 0.04, 0.08]
        else:  # Responsive
          lpf_accel = np.interp(CS.out.vEgo, [0, 10, 20], [0.012, 0.007, 0.0035])
          lpf_jerk = np.interp(CS.out.vEgo, [0, 10, 20], [0.2, 0.12, 0.02])
          deviation_curve = [0.018, 0.045, 0.09]
        self.accel_diff = (lpf_accel * (accel - self.accel_last)) + (1 - lpf_accel) * self.accel_diff
        self.long_jerklimit = (lpf_jerk * np.clip(abs(accel), 0.7, 2.0)) + (1 - lpf_jerk) * self.long_jerklimit
        self.long_deviation = np.interp(abs(accel - self.accel_diff), [0, 0.3, 1.0], deviation_curve)
        self.accel_last = accel
