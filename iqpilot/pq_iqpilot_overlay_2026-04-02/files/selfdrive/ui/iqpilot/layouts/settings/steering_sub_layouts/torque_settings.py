...
from openpilot.system.ui.iqpilot.widgets.list_view import toggle_item, option_item
...
class TorqueSettingsLayout(Widget):
  ...
  def _initialize_items(self):
    self._self_tune_toggle = toggle_item(
    ...
    self._relaxed_tune_toggle = toggle_item(
    ...
    self._custom_tune_toggle = toggle_item(
    ...
    self._torque_prams_override_toggle = toggle_item(
    ...
    self._torque_lat_accel_factor = option_item(
    ...
    self._torque_friction = option_item(
  ...
  def _update_state(self):
    ...
    lat_factor = float(getattr(ltp, "latAccelFactorFiltered", 0.0))
    friction = float(getattr(ltp, "frictionCoefficientFiltered", 0.0))

    self_tune_desc = tr("Enables self-tune for Torque lateral control for platforms that do not use Torque lateral control by default.")
    if ui_state.params.get_bool("LiveTorqueParamsToggle"):
      status = tr("Active") if live_active else tr("Waiting")
      validity = tr("valid") if live_valid else tr("learning")
      self_tune_desc += f"<br><br>{tr('Status')}: {status} | {tr('Calibration')}: {cal_perc}% | {tr('Output')}: {validity}"
      self_tune_desc += f"<br>{tr('Learned LAF')}: {lat_factor:.3f} | {tr('Learned Friction')}: {friction:.3f}"
    self._self_tune_toggle.set_description(self_tune_desc)

    relaxed_desc = tr("Less strict settings when using Self-Tune. This allows torqued to be more forgiving when learning values.")
    if ui_state.params.get_bool("LiveTorqueParamsRelaxedToggle"):
      relaxed_desc += f"<br><br>{tr('Use this only if calibration stalls below 100% or varies too much on your car.')}"
    self._relaxed_tune_toggle.set_description(relaxed_desc)
