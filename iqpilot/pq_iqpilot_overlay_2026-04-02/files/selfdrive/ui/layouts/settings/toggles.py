...
ELON_MODE_PATH = "/data/params/d/ElonMode"
...
class TogglesLayout(Widget):
  def __init__(self):
    ...
    for param, (title, desc, icon, needs_restart) in self._toggle_defs.items():
      initial_state = ui_state.elon_mode if param == "ElonMode" else self._params.get_bool(param)
      toggle = toggle_item(
        title,
        desc,
        initial_state,
  ...
  def _update_toggles(self):
    ...
    for param in self._toggle_defs:
      if param == "ElonMode":
        self._toggles[param].action_item.set_state(ui_state.elon_mode)
      else:
  ...
  def _toggle_callback(self, state: bool, param: str):
    if param == "ElonMode":
      ui_state.put_bool_file(ELON_MODE_PATH, state)
