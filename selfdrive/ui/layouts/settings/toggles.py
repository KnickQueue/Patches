...
DESCRIPTIONS = {
  ...
  "PerformanceMode": tr_noop("Reduce system overhead by disabling non-critical background services and prioritizing real-time threads."),
  ...
  "ElonMode": tr_noop("Bypass driver monitoring awareness checks."),
}
...
class TogglesLayout(Widget):
  def __init__(self):
    ...
    # Set Elon Mode to True by default in ui_state if not already set (param check handled in initialization script)
    # The UI toggle will follow the file-based parameter state, but we ensure the local state is also True if needed.
    if ui_state.elon_mode is None:
        ui_state.elon_mode = True

    # Default to IQ Standard drive mode if not configured
    if self._params.get("AlphaLongitudinalEnabled") is None:
        self._apply_longitudinal_control_mode(1) # Index 1 is IQ Standard
    ...
    self._toggle_defs = {
      ...
      "PerformanceMode": (
        lambda: tr("Performance Mode"),
        DESCRIPTIONS["PerformanceMode"],
        "speed_limit.png",
        True,
      ),
      "RecordFront": (
    ...
    for param, (title, desc, icon, needs_restart) in self._toggle_defs.items():
      ...
      if param == "ElonMode":
        toggle.action_item.set_state(ui_state.elon_mode)
  ...
  def _toggle_callback(self, state: bool, param: str):
    if param == "ElonMode":
      ui_state.put_bool_file("/data/params/d/ElonMode", state)
      ui_state.elon_mode = state
    else:
