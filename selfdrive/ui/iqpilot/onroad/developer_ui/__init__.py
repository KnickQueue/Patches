...
from openpilot.selfdrive.ui.iqpilot.onroad.developer_ui.elements import (
  ...
  DmFaceProbElement, DmEyeProbElement, DmBlinkProbElement, DmPhoneProbElement,
  DmAwarenessElement
)
...
class DeveloperUiRenderer(Widget):
  ...
  def __init__(self):
    ...
    self.dm_face_prob_elem = DmFaceProbElement()
    self.dm_eye_prob_elem = DmEyeProbElement()
    self.dm_blink_prob_elem = DmBlinkProbElement()
    self.dm_phone_prob_elem = DmPhoneProbElement()
    self.dm_awareness_elem = DmAwarenessElement()
  ...
  def _render(self, rect: rl.Rectangle) -> None:
    ...
    if ui_state.show_dm_debug:
      self._draw_dm_debug_ui(rect)
  ...
  def _draw_dm_debug_ui(self, rect: rl.Rectangle) -> None:
    sm = ui_state.sm
    ...
    UI_BORDER_SIZE = 20
    container_width = 184
    # Draw on the left side, mirroring the right dev UI but shifted left
    x = int(rect.x + UI_BORDER_SIZE * 2)
    y = int(rect.y + UI_BORDER_SIZE * 1.5)

    elements = [
      self.dm_face_prob_elem.update(sm),
      self.dm_eye_prob_elem.update(sm),
      self.dm_blink_prob_elem.update(sm),
      self.dm_phone_prob_elem.update(sm),
      self.dm_awareness_elem.update(sm),
    ]
