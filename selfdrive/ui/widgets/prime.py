...
class PrimeWidget(Widget):
  ...
  def __init__(self):
    ...
    self._icon_drives = gui_app.texture("icons/chffr_wheel.png", 52, 52, keep_aspect_ratio=True)
    self._icon_hours = gui_app.texture("icons/settings.png", 52, 52, keep_aspect_ratio=True)
  ...
  def _is_konn3kt_online(self) -> bool:
    ...
    # Athena/Konn3kt pings are Unix timestamps (seconds since epoch)
    # Convert current time to nanoseconds for consistent comparison
    now_ns = time.time() * 1e9
    return last_ping != 0 and (now_ns - last_ping) < self.KONN3KT_ONLINE_NS
