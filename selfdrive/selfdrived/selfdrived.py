...
class SelfdriveD(CruiseHelper):
  def __init__(self, CP=None, CP_IQ=None):
    ...
    ignore = self.sensor_packets + self.gps_packets + ['alertDebug', 'iqModelDataV2', 'iqNavState', 'liveParameters', 'driverAssistance']
    if SIMULATION:
    ...
    self.sm = messaging.SubMaster(['deviceState', 'pandaStates', 'peripheralState', 'modelV2', 'liveCalibration',
                                   ...
                                   'iqModelDataV2', 'iqNavState', 'iqPlan'] + \
  ...
  def data_sample(self):
    ...
    if self.enabled and any(not ps.controlsAllowed for ps in self.sm['pandaStates']
           if ps.safetyModel not in IGNORED_SAFETY_MODES and ps.safetyModel):
